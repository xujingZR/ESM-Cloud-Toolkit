...
async def nowtime(request_data: Request):
    ...
    module_name = "充放电数据基本设置"
    content_name_file = "文件"
    content_name_cycle = "目标循环数"

    r_dir = r"/home/battery/xu/plugin/landian/cycle_data"
    ...
    t_cycle = get_target_content_from_module_by_name(t_module, content_name_cycle)
    fnames, upload_token = download_files(file_content, t_dir = t_dir)
    t_file_path = [os.path.join(t_dir, fname) for fname in fnames]
    main_func(t_file_path, t_cycle, legend=fnames)

    name_id_1, uid_1 = upload_big_file(
        os.path.join(t_dir, "capacity_vs_cycles.png"),
        "capacity_vs_cycles.png",
        upload_token
    )

    name_id_2, uid_2 = upload_big_file(
        os.path.join(t_dir, "capacity_vs_voltage.png"),
        "capacity_vs_voltage.png",
        upload_token
    )
        ...
        "add":[
            {
                ...
                "type":"file",
                "name":"曲线",
                "data":["#file{%s}"%name_id_2, "#file{%s}"%name_id_1]
            }
        ],
        "message":["已完成"]
    }
